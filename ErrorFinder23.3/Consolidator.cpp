...
void Consolidator::setTrueMatches(std::vector < std::vector < std::vector < SNP > > > x)
{
    m_trueMatches = x;
}
    ...
    for(int j=i;j<person_count;j++)
    {
      for(int l=0;l<m_matches[i][j].size();l++)
      {
        ...
        if(temp1-extendSnp <0)
                                     ...
                                     if(temp2+extendSnp > 4443)// change this constant, it i the AGTC pair..
                                     {
                                      temp2=m_matches[i][j][l].end;
                                      //cout<<"New value of temp2= "<<temp2<<endl;
                                     }
        ...
        if( (e_obj.isInitialCmDrop(temp1,temp2,min_cm)) || ((temp2-temp1) < min_snp) ){ //initial drop. Don't calculate MA
          ...
          movingAverages = e_obj.getMovingAverages(finalErrors,temp1,temp2,window,extendSnp);//<piyush1> get moving averages are calculated from this part
